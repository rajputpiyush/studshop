...
session_start();
$idn = $_GET['search1'];
setcookie("name" , $idn , time()+60*60*24*10);
...
<?php
  ...
  $id = $_GET['search1'];
  // $new = strtolower($id);
  $sql = "SELECT * FROM `product` WHERE MATCH (title,desc1) against ('$id') LIMIT 60";
  // $sql = "SELECT * FROM `product` WHERE title LIKE '%$id%' ORDER BY offer_price";

$result = mysqli_query($conn , $sql);
$row = mysqli_num_rows($result);
echo '<h5><u>Total '.$row.' product is availble in this shop.</u></h5>';
