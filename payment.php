...
if($result){  
  ...
  while($rows = mysqli_fetch_assoc($result)){
    ...
    <p class="text-muted small">QUALITY: '.$qua.'<br> Each: '.$offer.'</p>
    </figcaption>
    </figure>
    </td>
    <td>
    <div class="price-wrap"> <var class="price">&#8377;'.$offers1.'</var> <small class="text-muted"> &#8377;<del>'.$prices1.'</del> </small> </div>
    </td>
 
    <td>
    <form action="payment.php?sno='.$sno.'" method="POST">
     
      <select id="inputState" class="form-control" name = "pro" style="width: 55px;">
      <option name ="pro" selected>'.$pro.'</option>
...
<?php
            ...
            $discount = $prices1 - $offers1;
            $charge1 = 50;
            $del = 50;
            $echarge1 = 40;
            $edel = 40;
	           echo' <div class="card" style="width: 300px; height: 430px;" id="divcard">
	                <div class="card-body">
	                    <dl class="dlist-align">
	                        <dt>Total price:</dt>
	                        <dd class="text-right ml-3">&#8377;'.$prices1.'</dd>
	                    </dl>
	                    <dl class="dlist-align">
	                        <dt>Discount:</dt>
                  ...
                  if($offers1 > 500){
                    echo'
                      <dl class="dlist-align">
                          ...
                          $tprices1 = $prices1 - $discount - $charge1 + $del - $echarge1;
      ...
      <input type="hidden" name="product_name" value="<?php echo $names;?>">
      ...
      <input type="number"  style="margin-bottom: 10px; margin-top: -40px; background-color: white; width: 260px; height: 40px; border-radius: 5px;" id="btn-remove12"
       name="phone" id="phone" placeholder=" Enter your contact no" required>
       <input type="submit" class="btn btn-out btn-primary btn-square btn-main" id="btn-remove12" data-abc="true" value="Click To Pay : Rs. <?php echo number_format($tprices1); ?>">
      
    </form>
      
      
    <a href="index.php" class="btn btn-out btn-success btn-square btn-main mt-2" id="btn-remove12" data-abc="true">Continue Shopping</a>
	                </div>
	            </div>
	        </aside>
	    </div>
...
?>
 onclick="return confirm('<?php echo $_SESSION['name'];?> are you sure you want to logout?');"
